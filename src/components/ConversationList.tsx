  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col h-full max-h-[850px] shadow-sm select-none">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    {editingSessionId === conv.id ? (
                      <div className="flex items-center gap-1.5 w-full mb-1" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="text"
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                              handleSaveSessionRename(conv.id);
                        ...
                        <button
                          type="button"
                          onClick={() => handleSaveSessionRename(conv.id)}
                          className="p-1 bg-green-50 hover:bg-green-100 text-green-600 rounded transition-colors cursor-pointer"
                          title="Save title"
                        >
      ...
      {isSettingsOpen && selectedGroupForSettings && (() => {
        ...
        const fhirMissing = FHIR_ANNOTATION_SCHEMA.filter(
          def => !schema.some(s => s.id === def.id || s.displayName.toLowerCase() === def.displayName.toLowerCase())
        );
        ...
        const handleAddFhirCategory = (fhirCatId: string) => {
          const found = FHIR_ANNOTATION_SCHEMA.find(def => def.id === fhirCatId);
          if (found) {
            updateSchema([...schema, found]);
          }
        };
                          ...
                          <div className="sm:col-span-6">
                            ...
                            >
                              <optgroup label="FHIR Standard Resources">
                                ...
                                <option value="Goal">Goal (Clinical & Behavioral Targets)</option>
                                <option value="MedicationRequest">MedicationRequest (Prescriptions & Orders)</option>
                                <option value="Medication">MedicationStatement (Usage / Adherence)</option>
                                <option value="ServiceRequest">ServiceRequest (Referrals & Orders)</option>
                                <option value="Condition">Condition (Disorders & Diagnoses)</option>
                                <option value="Symptom">Observation (Symptom / Subjective)</option>
                                <option value="Observation">Observation (Vital / Lab Measurement)</option>
                                <option value="Procedure">Procedure (Performed Interventions)</option>
                                <option value="AllergyIntolerance">AllergyIntolerance (Allergies)</option>
                                <option value="Immunization">Immunization (Vaccines)</option>
                                <option value="DiagnosticReport">DiagnosticReport (Panels & Reports)</option>
                                <option value="FamilyMemberHistory">FamilyMemberHistory (Pedigree)</option>
                              </optgroup>
                              <optgroup label="General Clinical Graphs">
